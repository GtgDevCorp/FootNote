...
main()
{
	...
	double adblData[16000] = {0};  //Max buffer size (#channels*numScansRequested)
	...
	lngErrorcode = ePut (lngHandle, LJ_ioPUT_ANALOG_ENABLE_PORT, 0, 255, 16);
	ErrorHandler(lngErrorcode, __LINE__, 0);
    ...
    ErrorHandler(lngErrorcode, __LINE__, 0);
	lngErrorcode = AddRequest(lngHandle, LJ_ioADD_STREAM_CHANNEL, 2, 0, 0, 0);
    ErrorHandler(lngErrorcode, __LINE__, 0);
    lngErrorcode = AddRequest(lngHandle, LJ_ioADD_STREAM_CHANNEL, 3, 0, 0, 0);
    ErrorHandler(lngErrorcode, __LINE__, 0);
	lngErrorcode = AddRequest(lngHandle, LJ_ioADD_STREAM_CHANNEL, 4, 0, 0, 0);
    ErrorHandler(lngErrorcode, __LINE__, 0);
    lngErrorcode = AddRequest(lngHandle, LJ_ioADD_STREAM_CHANNEL, 5, 0, 0, 0);
    ErrorHandler(lngErrorcode, __LINE__, 0);
	lngErrorcode = AddRequest(lngHandle, LJ_ioADD_STREAM_CHANNEL, 6, 0, 0, 0);
    ErrorHandler(lngErrorcode, __LINE__, 0);
    lngErrorcode = AddRequest(lngHandle, LJ_ioADD_STREAM_CHANNEL, 7, 0, 0, 0);
    ErrorHandler(lngErrorcode, __LINE__, 0);
	...
	while(!kbhit())	//Loop will run until any key is hit
	{
		...
		printf("First scan = %.3f, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f\n",adblData[0],adblData[1],adblData[2],adblData[3],adblData[4],adblData[5],adblData[6],adblData[7]);
		ErrorHandler(lngErrorcode, __LINE__, 0);
		//Retrieve the current U3 backlog.  The UD driver retrieves stream data from
		...
		printf("U3 Backlog = %.1f%%\n",(100.0*dblCommBacklog/256.0));
		//Retrieve the current UD backlog.  The UD driver retrieves stream data from
		//the U3 in the background, but if the computer is too slow for some reason
		//the driver might not be able to read the data as fast as the U3 is
		//acquiring it, and thus there will be data left over in the U3 buffer.
		lngErrorcode = eGet(lngHandle, LJ_ioGET_CONFIG, LJ_chSTREAM_BACKLOG_UD, &dblUDBacklog, 0);
		printf("UD Backlog = %.1f%%\n",(100*dblUDBacklog/(scanRate*2*5)));
		i++;
	}
